...
struct fasync_struct *fasync_buttons;
...
{
	unsigned int i;
	struct pin_desc *pbutton = irq_pd;
	if(!pbutton)
		return;
	...
	kill_fasync(&fasync_buttons, SIGIO, POLL_IN);
}
...
static unsigned int poll_buttons(struct file *file, struct poll_table_struct *wait)
{
	unsigned int mask = 0;
...
{
	printk("fasync init\n");
	return fasync_helper(fd, filp, on, &fasync_buttons);;
}
